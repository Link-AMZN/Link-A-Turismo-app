-- migration_001_professional_hotel_system_corrected.sql
...
CREATE INDEX IF NOT EXISTS hotels_location_idx ON hotels USING GIST(location_geom);
CREATE INDEX IF NOT EXISTS hotels_host_idx ON hotels(host_id);
...
CREATE INDEX IF NOT EXISTS rooms_room_type_idx ON rooms(room_type_id);
CREATE INDEX IF NOT EXISTS room_availability_room_type_date_idx ON room_availability(room_type_id, date);
CREATE INDEX IF NOT EXISTS room_availability_hotel_date_idx ON room_availability(hotel_id, date);
CREATE INDEX IF NOT EXISTS rate_plans_hotel_idx ON rate_plans(hotel_id);
CREATE INDEX IF NOT EXISTS rate_plan_rules_dates_idx ON rate_plan_rules(from_date, to_date);
...
CREATE OR REPLACE TRIGGER update_hotels_updated_at BEFORE UPDATE ON hotels FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE OR REPLACE TRIGGER update_room_types_updated_at BEFORE UPDATE ON room_types FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE OR REPLACE TRIGGER update_rooms_updated_at BEFORE UPDATE ON rooms FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE OR REPLACE TRIGGER update_rate_plans_updated_at BEFORE UPDATE ON rate_plans FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
