-- Corrigir a função search_hotels_smart - VERSÃO FINAL COMPLETA
...
BEGIN
    ...
    available_hotels AS (
        SELECT 
            h.id as available_hotel_id,  -- Mudei o nome para evitar conflito
            h.name as available_hotel_name,
            h.slug as available_hotel_slug,
            h.description as available_description,
            h.address as available_address,
            h.locality as available_locality,
            h.province as available_province,
            h.lat as available_lat,
            h.lng as available_lng,
            ...
            COALESCE(
                ...
                '[]'::jsonb
            ) as available_room_types_json,
            
            ras.min_price as available_min_price,
            ras.max_price as available_max_price,
            ...
            CASE
                ...
                ELSE 0
            END as available_base_score,
            
            CASE 
                WHEN required_amenities IS NOT NULL AND h.amenities @> required_amenities THEN 20
                ELSE 0
            END as available_amenities_bonus,
            
            COUNT(DISTINCT rt.id) as available_total_rooms
        ...
        WHERE h.is_active = true
          ...
          AND (max_price_param IS NULL OR ras.min_price <= max_price_param)
    ...
    SELECT 
        available_hotel_id,           -- Use os nomes corrigidos
        available_hotel_name,
        available_hotel_slug,
        ...
        available_lat,
        available_lng,
        available_distance_km,
        available_room_types_json,
        available_min_price,
        available_max_price,
        (available_base_score + available_amenities_bonus) as match_score,
        available_total_rooms
        
    FROM available_hotels
    WHERE available_total_rooms > 0
      AND (available_base_score > 0 OR search_radius_km = 0)
    ORDER BY 
        match_score DESC,
        available_distance_km ASC,
        available_min_price ASC
    LIMIT max_results;
